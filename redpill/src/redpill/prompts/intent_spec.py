...
CATEGORICAL_FIELDS_PLACEHOLDER = "categorical_fields"
TIME_FIELDS_PLACEHOLDER = "time_fields"
NUMERIC_FIELDS_PLACEHOLDER = "numeric_fields"
ALL_FIELDS_PLACEHOLDER = "all_fields"
PROMPT_PLACEHOLDER = "prompt"
PROFILE_PLACEHOLDER = "profile"
SAMPLE_DATA_PLACEHOLDER = "sample_data"
SAMPLE_SIZE_PLACEHOLDER = "sample_size"
...
def build_intent_spec_prompt(
    ...
    sample_json = "\n".join(str(row) for row in sample_data[:sample_size])

    user_prompt = INTENT_SPEC_USER_PROMPT_TEMPLATE.replace(
        PROMPT_PLACEHOLDER, prompt
    ).replace(
        PROFILE_PLACEHOLDER, str(profile)
    ).replace(
        SAMPLE_DATA_PLACEHOLDER, sample_json
    ).replace(
        SAMPLE_SIZE_PLACEHOLDER, str(sample_size)
    ).replace(
        CATEGORICAL_FIELDS_PLACEHOLDER, ", ".join(categorical_fields) or "None"
    ).replace(
        TIME_FIELDS_PLACEHOLDER, ", ".join(time_fields) or "None"
    ).replace(
        NUMERIC_FIELDS_PLACEHOLDER, ", ".join(numeric_fields) or "None"
    ).replace(
        ALL_FIELDS_PLACEHOLDER, ", ".join(all_fields) or "None"
    )
